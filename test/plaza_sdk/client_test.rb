...
class PlazaSDKTest < Minitest::Test
  ...
  def test_raises_on_unknown_environment
    e = assert_raises(ArgumentError) do
      PlazaSDK::Client.new(environment: "wrong")
    end
  ...
  def test_client_default_request_default_retry_attempts
    stub_request(:get, "http://localhost/api/v1/features/nearby").to_return_json(status: 500, body: {})

    plaza = PlazaSDK::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(PlazaSDK::Errors::InternalServerError) do
      plaza.elements.nearby(lat: 0, lng: 0)
    end
  ...
  def test_client_given_request_default_retry_attempts
    ...
    plaza = PlazaSDK::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(PlazaSDK::Errors::InternalServerError) do
      plaza.elements.nearby(lat: 0, lng: 0, request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(PlazaSDK::Errors::InternalServerError) do
      plaza.elements.nearby(lat: 0, lng: 0, request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:get, "http://localhost/api/v1/features/nearby").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    plaza = PlazaSDK::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(PlazaSDK::Errors::APIConnectionError) do
      plaza.elements.nearby(lat: 0, lng: 0, request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: PlazaSDK::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: PlazaSDK::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: PlazaSDK::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:get, "http://localhost/api/v1/features/nearby").to_return_json(status: 200, body: {})
